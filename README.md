...
- **기능 구현 전 서비스 흐름을 구조화하는 설계 습관 형성**  
  기능 단위로 구현을 시작하기보다 데이터 흐름과 사용자 동선을 먼저 정리하는 과정이  
  개발 과정의 오류를 줄이고 구현 속도를 높인다는 것을 경험했습니다.

- **외부 API 연동 과정에서의 요청–응답 구조 이해**  
  소셜 로그인과 결제 기능을 구현하며  
  API 문서를 기반으로 요청 방식, 파라미터, 응답 데이터 구조를 분석하고  
  이를 서비스 로직에 연결하는 방법을 익혔습니다.

- **협업 기반 개발 프로세스 경험**  
  팀 프로젝트를 진행하며 기능 단위 역할 분담, 코드 공유, Git 기반 협업을 통해  
  여러 개발자가 동시에 작업하는 환경에서 협업 구조와 개발 흐름의 중요성을 경험했습니다.
